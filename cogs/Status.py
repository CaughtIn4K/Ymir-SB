...
class Status(commands.Cog):
      ...
      async def stream(self, ctx, *, text):
       ...
       await self.client.change_presence(activity=discord.Streaming (url = "https://www.twitch.tv/reallsolar", name= text))
       embed = discord.Embed(description = f"**{ctx.author.mention}, Is now streaming {text}**", color=random.choice(self.colors))
      ...
      async def stop(self, ctx):
       ...
       embed = discord.Embed(description = f"**{ctx.author.mention}'s Status Has Been Resetted**", color=random.choice(self.colors))
       await ctx.send(embed=embed)
